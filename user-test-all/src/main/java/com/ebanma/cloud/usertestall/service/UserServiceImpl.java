...
/**
 * @author 肖露
 * @version $ Id: UserServiceImpl, v 0.1 2023/03/20 8:39 banma-0241 Exp $
 */
@Service
public class UserServiceImpl implements UserService {
    ...
    public PageResult<List<UserDTO>> query(PageQuery<UserQueryDTO> pageQuery) {
        ...
        UserDO userDO = new UserDO();
        BeanUtils.copyProperties(pageQuery.getQuery(), pageQuery.getPageSize());
        ...
        List<UserDTO> userDTOList = Optional.ofNullable(userPage.getRecords()).map(List::stream).orElseGet(
            Stream::empty).map(user -> {
